...
def MakePlayerMove(Board, Ships):
  ...
  if Board[Row][Column] == "m" or Board[Row][Column] == "h":
    ...
    Board[Row][Column] = Board[Row][Column].lower()
    for r in Board:
      for i in r:
        if Board[Row][Column].upper() == i:
          r[r.index(i)] = Board[Row][Column]
          Hits += 1
    Hits += 1; Moves += 1 

  print("Moves: {0} | Hits: {1} | Misses: {2}".format(Moves, Hits, Misses))
