...
def MakePlayerMove(Board, Ships):
  ...
  if Row == "s" or Column == "s":
    ...
    saveGame(Board, fName)
    exit()
  elif Board[Row][Column] == "m" or Board[Row][Column] == "h":
...
def saveGame(Board, fName):
  save = open("{0}.txt".format(fName), "w")
  for Row in Board:
    for i in Row:
      save.write(i)
    save.write("\n")
...
if __name__ == "__main__":
  ...
  while not MenuOption == 9:
    ...
    if MenuOption == 2:
      ...
      PlayGame(Board, Ships)
    if MenuOption == 3:
      lName = str(input("Enter the game to load: "))
      LoadGame("{0}.txt".format(lName), Board)
      PlayGame(Board, Ships)